...
internal class CardPaymentPresenter(
    ...
    private lateinit var savedCardPaymentUrl: String
    ...
    private var savedCardPayment : SavedCardPayment = SavedCardPayment()

    constructor(url: String,
                code: String,
                view: CardPaymentContract.View,
                interactions: CardPaymentContract.Interactions,
                paymentApiInteractor: PaymentApiInteractor,
                stringResources: StringResources) :
        this(view, interactions, paymentApiInteractor, stringResources) {
        this.url = url
        this.code = code
    ...
    private val cardDetector: CardDetector by lazy {
        if (supportedCards.isEmpty()) {
            supportedCards = CardMapping.mapSupportedCards(listOf("VISA", "MASTERCARD", "AMERICAN_EXPRESS"))
        }
    ...
    internal fun onHandlePaymentAuthorization(cookies: List<String>, orderUrl: String) {
        ...
        paymentApiInteractor.getOrder(
                ...
                success = { orderReference, paymentUrl, supportedCards, orderAmount, savedCardPaymentUrl ->
                    view.showProgress(false)
                    this.orderReference = orderReference
                    this.paymentUrl = paymentUrl
                    this.savedCardPaymentUrl = savedCardPaymentUrl
                    this.supportedCards = supportedCards
                    this.orderAmount = orderAmount
                    if (isEmbedded) {
                        if (savedCardPayment.isInitialized) {
                            executeSavedCardPayment()
                        } else {
                            executePayment()
                        }
                    } else {
    ...
    private fun executeSavedCardPayment() {
        paymentApiInteractor.doSavedCardPayment(
                savedCardPaymentUrl = savedCardPaymentUrl,
                paymentCookie = paymentCookie,
    ...
    override fun pay(url: String, code: String, savedCardPayment: SavedCardPayment) {
        this.url = url
        this.code = code
    ...
    override fun getCardholderName() : String {
        return view.cardHolder.rawTxt
    }

    override fun getCardNumber() : String {
        return view.cardNumber.rawTxt.substring(12)
    }

    override fun getExpiry() : String {
        return DateFormatter.formatExpireDateForApi(view.expireDate.rawTxt)
    }

    override fun getCardType() : CardType {
//        val detect = cardDetector.detect(view.cardNumber.rawTxt)
        return paymentCard?.let { card -> card.type } ?: CardType.Visa
    }
